-- xmobar.hs
-- Last update: 2023-05-05 12:21:35 (CEST)
...
Config {
   ...
   template = " <icon=haskell_20_dark.xpm/> %UnsafeStdinReader% }{ [ %cpu% %coretemp% | %memory% %swap% | %dynnetwork% | %battery% | %date% ] %spaces1%",
   ...
   commands =
     [
       ...
       -- theme: default / blue
       Run Date              "<icon=calendar.xbm/> <fc=#268bd2>%a %Y-%m-%d %H:%M:%S</fc>" "date" 10, -- 1s
