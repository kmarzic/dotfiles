-------------------------------------------------------------------------------
-- xmonad.hs
-- Last update: 2023-05-05 12:23:58 (CEST)
-------------------------------------------------------------------------------
...
myTabConfigGruvbox :: Theme -- theme: gruvbox
myTabConfigGruvbox = def
  {
    activeColor = gruvbox M.! "comment",
    activeTextColor = gruvbox M.! "foreground",
    activeBorderColor = gruvbox M.! "purple",
    inactiveColor = gruvbox M.! "line",
    inactiveTextColor = gruvbox M.! "foreground",
    inactiveBorderColor = gruvbox M.! "foreground",
    urgentColor = gruvbox M.! "red",
    urgentTextColor = gruvbox M.! "foreground",
    urgentBorderColor = gruvbox M.! "green",
    fontName = fontBold
  }
...
myXmobarLogHookGruvboxPP :: PP -- theme: gruvbox
myXmobarLogHookGruvboxPP = def
  {
    ...
    ppOrder           = \(ws:l:t:ts:ex) -> [ws,l,"[",xmobarColor (gruvbox M.! "red") "" ts,"]",t] ++ ex ++ []
  }

myDzen2LogHookGruvboxPP :: PP -- theme: gruvbox
myDzen2LogHookGruvboxPP = def
  {
    ppCurrent         = dzenColor (gruvbox M.! "cyan") "" . wrap "[" "]",
    ppHidden          = dzenColor (gruvbox M.! "foreground") "",
    ppHiddenNoWindows = dzenColor (gruvbox M.! "comment") "",
    ppTitle           = dzenColor (gruvbox M.! "cyan") "" . shorten 50,
    ppVisible         = wrap "(" ")",
    ppUrgent          = dzenColor (gruvbox M.! "red") (gruvbox M.! "yellow"),
    ppLayout          = dzenColor (gruvbox M.! "green") "" . (\layout -> myPPLayout (layout)),
    ppSep             = " ", -- separator between each object
    ppWsSep           = " ", -- separator between workspaces
    -- (1)
    -- ppExtras          = [ logTitles ],
    ...
    -- (2)
    ppExtras          = [ windowCount ],
    ppOrder           = \(ws:l:t:ts:ex) -> [ws,l,"[",dzenColor (gruvbox M.! "red") "" ts,"]",t] ++ ex ++ []
  }

-- gruvbox

myXmobarLogHookGruvbox1 :: Handle -> X()
myXmobarLogHookGruvbox1 h = dynamicLogWithPP myXmobarLogHookGruvboxPP
  {
    ppOutput = hPutStrLn h
  }

myXmobarLogHookGruvbox2a :: Handle -> ScreenId -> PP
myXmobarLogHookGruvbox2a h s = myXmobarLogHookGruvboxPP
  {
    ppOutput = hPutStrLn h
  }

myXmobarLogHookGruvbox2 :: [Handle] -> ScreenId -> X ()
myXmobarLogHookGruvbox2 hs ns = mapM_ dynamicLogWithPP $ zipWith myXmobarLogHookGruvbox2a hs [0..ns-1]

myDzen2LogHookGruvbox1 :: Handle -> X()
myDzen2LogHookGruvbox1 h = dynamicLogWithPP myDzen2LogHookGruvboxPP
  {
    ppOutput = hPutStrLn h
  }

myDzen2LogHookGruvbox2a :: Handle -> ScreenId -> PP
myDzen2LogHookGruvbox2a h s = myDzen2LogHookGruvboxPP
  {
    ppOutput = hPutStrLn h
  }

myDzen2LogHookGruvbox2 :: [Handle] -> ScreenId -> X ()
myDzen2LogHookGruvbox2 hs ns = mapM_ dynamicLogWithPP $ zipWith myDzen2LogHookGruvbox2a hs [0..ns-1]
...
myKeysDmenuCommandGruvbox =
  [
    ((myModMask,                 xK_p      ), spawn dmenuCommandGruvbox), -- theme: gruvbox
    ((0,                         xK_Menu   ), spawn rofiCommand)  -- theme: gruvbox
  ]
...
myConfigGruvbox xmobar nScreens = myConfigDefault -- theme: gruvbox
    {
      normalBorderColor    = myNormalBorderColorGruvbox,
      focusedBorderColor   = myFocusedBorderColorGruvbox,
      layoutHook           = myLayoutHook myTabConfigGruvbox,
      -- (1) single xmobar
      -- logHook              = myXmobarLogHookGruvbox1 xmobar
      -- (2) multiple xmobar
      logHook              = updatePointer (0.5, 0.5) (0, 0) >> myXmobarLogHookGruvbox2 xmobar nScreens
      -- (3) multiple dzen2
      -- logHook              = updatePointer (0.5, 0.5) (0, 0) >> myDzen2LogHookGruvbox2 xmobar nScreens
    } `additionalKeys` myKeysDmenuCommandGruvbox
...
main = do
  ...
  -- xmonad $ myConfigGruvbox xmobar1 1 -- theme: gruvbox
  --
  -- (2) multiple xmobar
