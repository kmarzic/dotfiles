-- xmobar.hs
-- Last update: 2021-01-05 08:36:08 (CET)
...
Config {
   ...
   -- font = "xft:Liberation Mono:size=12:antialias=true:autohint=true:style=regular",
   ...
   commands =
     [
       ...
       -- theme: default / cyan
       -- Run Date              "<icon=clock.xbm/> <fc=#88C0D0>%a %Y-%m-%d %H:%M:%S</fc>" "date" 10, -- 1s
       Run Date              "<icon=calendar.xbm/> <fc=#88C0D0>%a %Y-%m-%d %H:%M:%S</fc> <icon=clock.xbm/>" "date" 10, -- 1s
